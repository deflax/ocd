...
- Pre-installed tools: git, ripgrep, fzf, curl, Python, BasedPyright, CMake/CTest, Terraform, Terraform LS, Node.js language servers, and more
...
The image includes Python as both `python3` and `python`, BasedPyright's `basedpyright-langserver`, CMake/CTest tooling, and Terraform tooling as both `terraform` and `terraform-ls`, so OpenCode can run common Python tests, Python LSP diagnostics, CMake/CTest workflows, Terraform formatting, and Terraform LSP diagnostics inside the container after rebuilding with `./build`.
