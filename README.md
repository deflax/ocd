...
The image includes Python as both `python3` and `python`, BasedPyright's `basedpyright-langserver`, CMake/CTest tooling, Terraform tooling as both `terraform` and `terraform-ls`, and Playwright with Chromium installed under the shared `/ms-playwright` browser path. OpenCode can run common Python tests, Python LSP diagnostics, CMake/CTest workflows, Terraform formatting, Terraform LSP diagnostics, and headless Chromium browser QA inside the container after rebuilding with `./build`.
